...
if (typeof document === "undefined" && typeof importScripts !== "undefined") {
  // test if we are in worker scope
  importScripts("clipper.js");

  self.addEventListener(
    "message",
    function (
      e // We listen messages from the main page
    ) {
      var data = e.data;
...
function draw(count) {
  ...
  var svg = "",
    offsetted_paths = new ClipperLib.Path(),
    i,
    xstart,
    ystart,
    xend,
  ...
  svg +=
    '<svg style="margin-top:10px;margin-right:10px;margin-bottom:10px;background-color:#dddddd" width="800" height="320">';
  for (i = 0; i < count; i++) {
    ...
    svg +=
      ...
      randomColor({ red: 255, green: 255, blue: 255 }) +
      '" stroke-width="1" d="' +
      paths2string(offsetted_paths, scale) +
      '"/>';
  }
  svg += "</svg>";
  return svg;
